...
def analyze_fraud(receiver_upi: str, amount: float, device_id: str, user_id: int):
    ...
    if device:
        ...
        layer3 = 75
        cur.execute(
            "INSERT INTO devices (user_id, device_fingerprint, trust_score) VALUES (%s, %s, %s)",
            (user_id, device_id, 25)
        )
    ...
    status = "blocked" if risk_score > 35 else "unsafe"
...
def analyze_transaction(user_id: int, txn: TransactionCreate):
    ...
    try:
        cur.execute(
            ...
            (user_id, receiver_upi, amount, device_id, risk_score, status, fraud_reason)
            VALUES (%s, %s, %s, %s, %s, %s, %s) RETURNING id""",
            (user_id, txn.receiver_upi, txn.amount, txn.device_id,
             result["risk_score"], result["status"], result["fraud_reason"])
        )
        ...
        if result["status"] == "blocked":
            cur.execute(
                ...
                (transaction_id, layer1_score, layer2_score, layer3_score, layer4_score, explanation)
                VALUES (%s, %s, %s, %s, %s, %s)""",
                (txn_id, result["layer1"], result["layer2"],
                 result["layer3"], result["layer4"], result["fraud_reason"])
            )
...
def get_history(user_id: int):
    ...
    try:
        cur.execute(
            """SELECT id, receiver_upi, amount, risk_score, status, fraud_reason, transaction_time 
            FROM transactions WHERE user_id = %s ORDER BY transaction_time DESC""",
            (user_id,)
        )
        rows = cur.fetchall()
        return [
...
def get_stats(user_id: int):
    ...
    try:
        ...
        return {
            ...
            "frauds_blocked": blocked,
            "amount_saved": float(saved)
        }
    finally:
        cur.close()
        conn.close()